...
async function tavilySearch(query: string, num: number, opts?: { days?: number; depth?: 'basic'|'advanced' }): Promise<Result[]> {
  ...
  if (!apiKey) return [];
  // Scope search to Apify documentation
  const siteQuery = `site:docs.apify.com ${query}`;
  const body: any = { query: siteQuery, max_results: num };
...
export const apifySearchTool = new DynamicTool({
  name: 'apify_search',
  description: 'Search Apify documentation (docs.apify.com) and return JSON results: [{title, url, snippet, content?}]. Use this to find information about Actors, SDK, API, platform features, tutorials, and examples.',
